...
Галочкой `обязательно` в интерфейсе UGE отмечены первые четыре позиции; остальные появляются в форме, но нужны в зависимости от конкретного кейса.
...
Эти документы не показаны на экране UGE как отдельные строки, но могут входить в пакет и загружаться через раздел **«Остальные документы»**:
