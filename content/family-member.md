...
| **Formulario MI-F**<br>Отдельное заявление на каждого члена семьи. [Официальная форма MI-F](https://ciudadaniaexterior.inclusion.gob.es/web/migraciones/modelos-de-solicitudes-de-la-ley-14/2013). |
| **Tasa 790 038 — 73,26 €**<br>На каждого заявителя оплачивается отдельная tasa, прикладывается подтверждение оплаты. |
| **Документ о родстве**<br>Для супруга — свидетельство о браке, для ребенка — свидетельство о рождении. **Российские документы ЗАГС не требуют апостиля для Испании**, но для UGE нужен [`jurado` перевод](../docs/document-requirements.html#jurado-perevod). [Подробнее ↓](#family-link) |
| **Справка о несудимости — для совершеннолетнего, если требуется**<br>Российская справка о несудимости — **с апостилем + `jurado` переводом**. Для лиц младше 18 лет справка не нужна. [Подробнее](../docs/criminal-record-certificate.html) |
| **Declaración responsable об отсутствии судимости за последние 5 лет**<br>Отдельный документ из общего списка UGE. На испанском, перевод не нужен. Для несовершеннолетних UGE прямо отменяет только справку о несудимости; поэтому декларацию из детского expediente автоматически не исключаем. |
...
| **Подтверждение дохода титуляра — зависит от момента подачи**<br>При одновременной подаче достаточно, чтобы титуляр подтвердил нужный доход на всю семью. Если член семьи присоединяется позже, доход титуляра подтверждается заново. [Подробнее ↓](#family-income) |
| **Медицинская страховка — только когда она действительно нужна**<br>**Если титуляр подается по контракту / как autónomo и после одобрения встает в испанскую Seguridad Social, отдельный частный полис для обычного супруга или ребенка не нужен.** В типичном кейсе титуляра **по найму**, если нет отдельного документа о праве семьи на государственное медицинское покрытие в Испании, нужен частный полис. [Подробнее ↓](#family-insurance) |
| **Declaración jurada de mantenimiento del vínculo familiar — не базовый документ первичной подачи**<br>В старых пакетах и при продлении этот документ встречается, но в актуальной инструкции UGE для `inicial familiares` он не указан как обязательный. Прикладывать его стоит, если UGE запросила подтверждение сохранения семейной связи или если есть отдельное основание. |
...
1. `Passport_familiar.pdf` — полный паспорт, все страницы.
2. `MI-F.pdf` — заявление члена семьи.
3. `Tasa_familiar.pdf` — tasa 790 038 и подтверждение оплаты.
4. `Marriage_certificate.pdf` — российское свидетельство о браке **без апостиля** + `jurado` перевод.
...
- `Declaración jurada de mantenimiento del vínculo familiar` **не входит в актуальный обязательный список inicial UGE**. Ее можно приложить дополнительно или по запросу UGE.

Такой набор соответствует логике актуальной инструкции UGE и хорошо объясняет, почему в реальном семейном пакете при одновременной подаче с autónomo может не быть ни отдельной страховки, ни повторного блока доходов.
...
1. паспорт ребенка;
2. отдельный MI-F;
3. отдельная tasa 790 038;
4. свидетельство о рождении + `jurado` перевод;
...
- `Declaración jurada de mantenimiento del vínculo familiar` не включена в актуальный обязательный список UGE для initial familiares; может использоваться дополнительно, по запросу или в процедурах продления.
